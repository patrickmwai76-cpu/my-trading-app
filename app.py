...
st.markdown("""
    ...
    .glass-card {
        ...
        margin-bottom: 20px; box-shadow: 0 8px 32px rgba(0,0,0,0.8);
    }
    div.stButton > button {
    ...
    /* Fix table background for dark mode sidebar */
...
with st.sidebar:
    ...
    lots = max((bal * (risk_pct/100)) / (sl_pips * 10), 0.01) if sl_pips > 0 else 0.01
    st.success(f"Lot Size: **{lots:.2f}**")
    
    st.divider()
    tf = st.radio("Main Chart TF", ["1m", "5m", "15m"], index=1, horizontal=True)
    
    # Placeholder for the Live Matrix in the Sidebar
    matrix_placeholder = st.empty()

# --- 4. THE LIVE ENGINE ---
@st.fragment(run_every=7)
def dashboard_engine():
    # 4a. TREND MATRIX CALCULATION
    matrix_data = []
    ...
    # Update Sidebar Matrix without stacking
    ...
    if data is not None and not data.empty:
        ...
        data['MACD_H'] = ta.macd(data['Close'])['MACDh_12_26_9']
        ...
        final_conf = int(((bias_count / 3) * 40) + ((sop_score / 3) * 40) + (20 if last['Raw'] != 0 else 0))
        
        adx_val = last['ADX']
        s_txt, s_clr = ("STRONG", "#00FF88") if adx_val > 25 else ("WEAK", "#777")
        ...
        with st_col:
            st.markdown(f'<div class="glass-card" style="border-bottom:4px solid {s_clr};">STRENGTH<br><h1 style="color:{s_clr};">{int(adx_val)}</h1>{s_txt}</div>', unsafe_allow_html=True)

        # BUTTONS
        c1, c2 = st.columns(2)
        c1.button("🚀 EXECUTE BUY", key="buy_btn")
        c2.button("📉 EXECUTE SELL", key="sell_btn")
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"chart_v116_{choice}")

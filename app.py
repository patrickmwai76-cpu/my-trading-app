...
# --- 4. TOP STATS ---
sig = "BUY" if df['Close'].iloc[-1] > df['EMA20'].iloc[-1] else "SELL"
sig_c = "#00ff00" if sig == "BUY" else "#ff4b4b"

st.markdown(f"<h1 style='text-align:center; color:{sig_c};'>SIGNAL: {sig}</h1>", unsafe_allow_html=True)
...
# Price & EMA 20 (The Trend Line)
fig.add_trace(go.Candlestick(
    x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name='US30'
), row=1, col=1)
fig.add_trace(go.Scatter(x=df.index, y=df['EMA20'], name='EMA 20', line=dict(color='orange', width=2)), row=1, col=1)
...
fig.update_layout(template='plotly_dark', height=

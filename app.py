...
if active_df is not None:
    ...
    st.markdown(f"""
        <div class="signal-card">
            <p style="color: #666; letter-spacing: 4px; font-size: 11px; margin-bottom:10px;">V11.6 TERMINAL ACTIVE</p>
            ...
            <div style="display: flex; justify-content: space-around; margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.05); padding-top:15px;">
                <div><p style="color:#444; margin:0; font-size:10px;">MOMENTUM</p><p style="font-size:20px; color:{trend_clr};">{trend_arrow} POWER</p></div>
...
# --- 6. CHARTING WITH "REAL SIGNAL" FILTER ---
if active_df is not None:
    # A. Logic: Identify Raw Signals
    active_df['Raw'] = 0
    active_df.loc[(active_df['Close'] > active_df['VWAP']) & (active_df['MACD_H'] > 0) & (active_df['ADX'] > 28) & (active_df['RSI'] < 70), 'Raw'] = 1
    active_df.loc[(active_df['Close'] < active_df['VWAP']) & (active_df['MACD_H'] < 0) & (active_df['ADX'] > 28) & (active_df['RSI'] > 30), 'Raw'] = -1
    
    # B. Filter: Only show the "First" signal of a trend change
    active_df['Clean_Signal'] = 0
    current_state = 0
    for i in range(len(active_df)):
        val = active_df.iloc[i]['Raw']
        if val != 0 and val != current_state:
            active_df.iloc[i, active_df.columns.get_loc('Clean_Signal')] = val
            current_state = val
        elif val == 0:
            current_state = 0
    ...
    # Plot "REAL" Buy/Sell Arrows
    buys = active_df[active_df['Clean_Signal'] == 1]
    fig.add_trace(go.Scatter(x=buys.index, y=buys['Low']*0.998, mode="markers+text", text="ENTRY", textposition="bottom center",
                             marker=dict(symbol="triangle-up", size=15, color="#00FF88", line=dict(width=1, color="white")), name="Buy"), row=1, col=1)
    
    sells = active_df[active_df['Clean_Signal'] == -1]
    fig.add_trace(go.Scatter(x=sells.index, y=sells['High']*1.002, mode="markers+text", text="ENTRY", textposition="top center",
                             marker=dict(symbol="triangle-down", size=15, color="#FF3366", line=dict(width=1, color="white")), name="Sell"), row=1, col=1)

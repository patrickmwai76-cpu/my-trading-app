...
st.title("🌌 PATRO AI PRO V12.0.0")

with st.sidebar:
    st.header("🏢 COMMAND CENTER")
    asset = st.selectbox("Select Asset", ["GOLD", "GBPUSD", "US30"])
    ticker_map = {"GOLD":"GC=F", "GBPUSD":"GBPUSD=X", "US30":"^DJI"}
    target_ticker = ticker_map[asset]
    
    st.divider()
    # THE SWITCH: Toggle between the old fast way and the new safe way
    mode = st.toggle("🚀 AGGRESSIVE MODE", value=False, help="ON: Signals every move. OFF: Only Bank moves.")
    
    st.divider()
    st.subheader("📰 GLOBAL NEWS")
    st.warning("⚠️ Market Impact: Trump Iran De-escalation")
    st.info("📉 Oil dropping, Gold testing liquidity")
...
def render_app():
    ...
    if df_5m is None or len(df_5m) < 15:
        st.error("Searching for Market Liquidity...")
        return
    ...
    sig_col = "#00FF88" if curr_sig == "BUY" else "#FF3366"
    display_col = sig_col if is_sure else "#FFA500"

    # Top Metrics
    m1, m2, m3 = st.columns([2, 1, 1])
    m1.markdown(f"<div style='border:2px solid {display_col}; padding:15px; border-radius:10px; background:#111; text-align:center;'><h1 style='color:{display_col}; margin:0;'>{status_text}</h1></div>", unsafe_allow_html=True)
    ...
    for i in range(1, limit):
        # Fair Value Gaps (Safety Fixed)
        if df_5m['FVG_Up'].iloc[i]:
            fig.add_shape(type="rect", x0=df_5m.index[i], x1=df_5m.index[i+2], y0=df_5m.High.iloc[i-1], y1=df_5m.Low.iloc[i+1], fillcolor="#00FF88", opacity=0.2, line_width=0)
        if df_5m['FVG_Down'].iloc[i]:
            fig.add_shape(type="rect", x0=df_5m.index[i], x1=df_5m.index[i+2], y0=df_5m.Low.iloc[i-1], y1=df_5m.High.iloc[i+1], fillcolor="#FF3366", opacity=0.2, line_width=0)
    ...
    rsi_fig.update_layout(height=150, template="plotly_dark", margin=dict(l=0,r=0,t=0,b=0), yaxis=dict(range=[0,100]))

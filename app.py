...
# 1. Page Setup & 30-Second Auto-Update
...
st.markdown("""
    ...
    .buy-mode { background: linear-gradient(90deg, #00c853 0%, #b2ff59 100%); padding: 15px; border-radius: 10px; color: black; font-weight: bold; text-align: center; margin-bottom: 20px; font-size: 22px; }
    </style>
    """, unsafe_allow_html=True)

# 2. Sidebar - All Control Tools
st.sidebar.title("🛡️ TERMINAL CONTROL")
tf = st.sidebar.radio("Timeframe", ["1m", "5m", "15m"], index=0, horizontal=True)

# SOP Checklist (Fixes your "Price Action near VWAP" check)
st.sidebar.divider()
...
# 3. Main Header
st.markdown('<div class="buy-mode">🛡️ PATRO AI PRO | INSTITUTIONAL TERMINAL v5.0</div>', unsafe_allow_html=True)

# 4. Market Data Engine
df = yf.download("^DJI", period="1d", interval=tf, group_by='column')

if not df.empty:
    # Standardize column headers to prevent crash
    df = df.copy()
    ...
    # VWAP Calculation (Price Action near VWAP feature)
    ...
    # 5. Triple-Row Dashboard (Price, RSI, Volume)
    fig = make_subplots(
        rows=3, cols=1, 
        shared_xaxes=True, 
    ...
    # --- ROW 1: CANDLES, VWAP, & FULL SIGNALS ---
    fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name='US30'), row=1, col=1)
    # Bright Blue VWAP Line
    fig.add_trace(go.Scatter(x=df.index, y=df['VWAP'], name='VWAP', line=dict(color='#00d2ff', width=2, dash='dot')), row=1, col=1)
    
    # Full "BUY" and "SELL" Spelling on every candle
    recent = df.tail(30)
    for i in range(len(recent)):
        if not pd.isna(recent['SMA_20'].iloc[i]):
            ...
            fig.add_annotation(x=recent.index[i], y=y_pos, text=label, showarrow=False, font=dict(color=color, size=8, family="Arial Black"), yshift=15 if is_buy else -15, row=1, col=1)
    ...
    # --- ROW 3: VOLUME BARS (Institutional View) ---
    ...
    st.error("Market Data Offline. Click Full System Refresh in the Sidebar.")

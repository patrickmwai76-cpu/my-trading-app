...
# 3. SIDEBAR: INSTITUTIONAL SOP & NEWS
with st.sidebar:
    ...
    # News for Wednesday, March 4, 2026
    events = [
...
try:
    ...
    if df.empty or len(df) < 20:
        st.warning("📡 SYNCING... Data is populating. Please wait 10 seconds.")
        st.stop()

    if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
    df = df.ffill().dropna() # Safety Fix for Black Screen
    ...
    # ADX Logic (With Division Safety)
    ...
    # --- SIDEBAR POWER METER WITH ARROWS ---
    with st.sidebar:
        st.divider()
        arrow = "▲" if last['ADX'] > prev['ADX'] else "▼"
        color = "green" if last['ADX'] > prev['ADX'] else "red"
        st.markdown(f"### ⚡ POWER: {last['ADX']:.1f}% :{color}[{arrow}]")

...
# --- 3. STUDIO HEADER ---
# This simulates the "African Man in Studio" look using a high-end visual banner
st.markdown(f"""
    <div style="background-color:#0d1117; padding:25px; border-radius:15px; border:2px solid {sig_color}; text-align:center; background-image: linear-gradient(to right, #0d1117, #1e2130);">
        <h1 style="margin:0; color:#ffffff; font-size:35px; letter-spacing:2px;">👨🏾‍💻 PATRO AI | OPERATOR STUDIO</h1>
        <p style="margin:0; color:grey; font-size:14px;">COMMAND CENTER: US30 INSTITUTIONAL FEED</p>
        <div style="margin-top:10px; display:inline-block; padding:5px 20px; border-radius:20px; background-color:{sig_color}; color:black; font-weight:bold;">
            {sig} SIGNAL ACTIVE
        </div>
    </div>
...
# --- 4. NEWS GUARD ---
st.markdown("<p style='margin-top:15px; color:#00ff00; font-weight:bold; font-size:12px;'>🛡️ NEWS GUARD ACTIVE</p>", unsafe_allow_html=True)
n1, n2 = st.columns(2)
...
fig.update_layout(template='plotly_dark', height=850, xaxis_rangeslider_visible=False, showlegend=False, margin=dict(l=10,r=10,t=10,b=10))
st.plotly_chart(fig, use_container_width=True)

# --- 6. SIDEBAR: OPERATOR SOP ---
with st.sidebar:

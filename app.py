...
# --- 4. NEWS GUARD (DANGER ZONE) ---
st.markdown("<p style='margin-top:10px; color:#00ff00; font-weight:bold;'>🛡️ NEWS GUARD ACTIVE</p>", unsafe_allow_html=True)
n1, n2 = st.columns(2)
with n1:
    st.info("**Mon Mar 2 @ 10:00 AM** - ISM Manufacturing PMI (High Impact)")
with n2:
    st.error("**Fri Mar 6 @ 08:30 AM** - Non-Farm Payrolls (CRITICAL)")

st.divider()

# METRICS
h1, h2, h3, h4 = st.columns(4)
h1.metric("PRICE", f"${df['Close'].iloc[-1]:,.2f}")
h2.metric("RSI", f"{df['RSI'].iloc[-1]:.2f}")
h3.metric("CONFLUENCE", f"{t1}/{t5}/{t15}")
h4.metric("VOL", f"{int(df['Volume'].iloc[-1]):,}")
...
fig.update_layout(template='plotly_dark', height=700, xaxis_rangeslider_visible=False, showlegend=False, margin=dict(l=10,r=10,t=0,b=10))
...
with st.sidebar:
    ...
    for l, v in [("1M", t1), ("5M", t5), ("15M", t15)]:
        ...
        st.markdown(f"<div style='border:1px solid {c}; padding:10px; border-radius:5px; margin-bottom:5px; color:{c};'><strong>{l}: {v}</strong></div>", unsafe_allow_html=True)
